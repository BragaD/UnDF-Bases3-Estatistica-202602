...
A seção 1.4 já esbarrou nisso: com $n = 27$, a posição do primeiro quartil é 7,5, e o valor saiu da média entre a sétima e a oitava observações. Aquela foi *uma* escolha entre várias. O argumento `interpolation` do pandas dá acesso a elas; vejamos três:

```{python}
#| label: convencoes-estados
convencoes = ["linear", "nearest", "midpoint"]
comp = pd.DataFrame(
...
Três convenções, e respostas diferentes para "qual é a distância interquartil da população dos estados": da menor (`{python} num(comp["dq"].min(), 0)`) à maior (`{python} num(comp["dq"].max(), 0)`), uma diferença de `{python} num(100 * (comp["dq"].max() / comp["dq"].min() - 1), 1)`%. Nenhuma está errada: `nearest` toma a observação mais próxima, `midpoint` o ponto médio entre as duas vizinhas, e `linear` (o padrão) interpola proporcionalmente à distância. Como aqui as posições caem exatamente no meio do caminho, `linear` e `midpoint` coincidem.

## Boxplot

O **boxplot** é uma leitura visual direta dos percentis. A caixa vai do primeiro ao terceiro quartil, e a largura dessa caixa é exatamente o IQR que a seção 1.4 calculou como medida de dispersão. É a mesma conta, olhada de dois ângulos diferentes. A linha dentro da caixa marca a mediana. Os bigodes se estendem até 1,5 vez o IQR além de cada quartil, e qualquer observação além disso é desenhada como um ponto individual, não absorvida pela escala do gráfico.

```{python}
#| label: fig-boxplot
#| fig-cap: "Boxplot da população dos estados. Os pontos acima do bigode são os estados atipicamente populosos."
#| fig-alt: "Boxplot vertical da população dos estados em milhões, com a caixa baixa e estreita perto de zero, o bigode superior mais longo que o inferior e dois pontos isolados bem acima do bigode."
...
#| fig-alt: "Boxplot vertical do aluguel em reais, com a caixa comprimida perto da base, bigode inferior curto e uma coluna densa de pontos acima do bigode superior, que se estende até alguns valores isolados muito acima dos demais."
