...
dentro = estado[(pop >= lim_inf) & (pop <= lim_sup)]
fora = estado[(pop < lim_inf) | (pop > lim_sup)].sort_values("Populacao")
bigode_inf = dentro.loc[dentro["Populacao"].idxmin()]   # menor valor dentro dos limites
bigode_sup = dentro.loc[dentro["Populacao"].idxmax()]   # maior valor dentro dos limites
```

```{python}
#| label: boxplot-milhoes
#| include: false
def mi(v, casas=1):
    return num(v / 1e6, casas).replace("-", "−")   # sinal de menos, não hífen

sigla_mediana = estado.loc[pop == q2, "Sigla"].iloc[0]
```

::: {.callout-note}
A construção dos bigodes e os limites $q_1 - 1{,}5\,d_q$ e $q_3 + 1{,}5\,d_q$ seguem a seção 3.4 de @bussab2023, que chama as observações fora deles de *pontos exteriores*.
:::

Cada traço do desenho marca um desses números:

::: {.table-responsive}
| Elemento do desenho | O que marca | Na população dos estados |
|---|---|---|
| Traço dentro da caixa | a mediana, $q_2$ | `{python} mi(q2, 3)` milhões (`{python} sigla_mediana`) |
| Lado esquerdo da caixa | o primeiro quartil, $q_1$ | `{python} mi(q1, 3)` milhões |
| Lado direito da caixa | o terceiro quartil, $q_3$ | `{python} mi(q3, 3)` milhões |
| Largura da caixa | a distância interquartil, $d_q = q_3 - q_1$ | `{python} mi(dq, 3)` milhões |
| Ponta do bigode da esquerda | a menor observação maior ou igual a $q_1 - 1{,}5\,d_q$ | `{python} num(bigode_inf["Populacao"] / 1e3, 0)` mil (`{python} bigode_inf["Sigla"]`) |
| Ponta do bigode da direita | a maior observação menor ou igual a $q_3 + 1{,}5\,d_q$ | `{python} mi(bigode_sup["Populacao"], 3)` milhões (`{python} bigode_sup["Sigla"]`) |
| Pontos isolados | as observações abaixo de $q_1 - 1{,}5\,d_q$ ou acima de $q_3 + 1{,}5\,d_q$ | `{python} " e ".join(fora["Sigla"])` |
:::

```{python}
#| label: fig-boxplot
#| fig-cap: "Boxplot da população dos estados. Os pontos à direita do bigode são os estados atipicamente populosos."
#| fig-alt: "Boxplot horizontal da população dos estados em milhões, com a caixa estreita perto de zero, o bigode da direita mais longo que o da esquerda e dois pontos isolados bem à direita do bigode."
fig, ax = plt.subplots(figsize=(7, 3))
ax.boxplot(estado["Populacao"] / 1e6, orientation="horizontal")
ax.set_yticks([])
...
::: {.exemplo}
Lendo o gráfico: metade dos estados tem entre `{python} mi(q1)` e `{python} mi(q3)` milhões de habitantes, e é isso que a caixa mostra. O bigode da direita **não** vai até o limite $q_3 + 1{,}5\,d_q$, que dá `{python} mi(lim_sup)` milhões: ele para no último estado antes dele, `{python} bigode_sup["Estado"]`. `{python} fora["Estado"].iloc[0]` (`{python} mi(fora["Populacao"].iloc[0])` milhões) e `{python} fora["Estado"].iloc[1]` (`{python} mi(fora["Populacao"].iloc[1])` milhões) passam do limite e viram pontos. Na outra ponta, o limite $q_1 - 1{,}5\,d_q$ dá `{python} mi(lim_inf)` milhões, uma população negativa, impossível. Nenhum estado fica abaixo dele, e o bigode da esquerda vai até o menor de todos, `{python} bigode_inf["Estado"]`. O limite cai abaixo de zero porque a caixa é larga e fica perto do chão: é a assimetria à direita que a seção 1.4 atribuiu ao piso em zero. Com ela, aqui só podem aparecer pontos isolados do lado direito.
:::
...
#| fig-alt: "Histograma da população dos estados em milhões com dez faixas, em que a primeira barra é de longe a mais alta, as seguintes diminuem rapidamente e uma única barra aparece isolada na extrema direita."
fig, ax = plt.subplots()
...
# assimetria, exatamente e não aproximadamente.
...
#| fig-alt: "Três histogramas de notas lado a lado, com o mesmo eixo vertical: na turma A a maior parte das notas fica à esquerda com cauda à direita e a média à direita da mediana, na turma B a imagem espelhada, e na turma C uma forma simétrica com média e mediana sobrepostas."
...
#| fig-alt: "Três boxplots horizontais de notas, um por turma: em A a mediana fica perto do lado esquerdo da caixa e o bigode direito é o mais longo, em B ocorre o espelho, e em C a mediana fica no centro da caixa com bigodes de comprimento parecido."
...
| Boxplot | $q_1$, mediana, $q_3$ e outliers num único desenho compacto |
| Tabela de frequência / histograma | Como a massa dos dados se distribui em faixas de largura igual |
| Assimetria | Para qual lado a distribuição pende, e o sinal da diferença entre média e mediana |
...
#| fig-cap: "Boxplot do aluguel. A caixa é o IQR; os pontos à direita do bigode são os outliers pela regra de 1,5×IQR."
#| fig-alt: "Boxplot horizontal do aluguel em reais, com a caixa comprimida perto do início do eixo, bigode da esquerda curto e uma faixa densa de pontos à direita do bigode da direita, que se estende até alguns valores isolados muito além dos demais."
...
fig, ax = plt.subplots(figsize=(7, 3))
ax.boxplot(alugueis["aluguel"], orientation="horizontal",
           flierprops={"markersize": 4, "alpha": 0.4})   # centenas de outliers: pontos pequenos e transparentes
ax.set_yticks([])
...
#| fig-cap: "Histograma da área só até o percentil 99: a forma aparece, com concentração nas áreas pequenas e cauda longa à direita, agora legível."
#| fig-alt: "Histograma da área dos imóveis em metros quadrados até o percentil 99, com pico nas áreas pequenas e barras que diminuem gradualmente para a direita, formando uma cauda longa."
fig, ax = plt.subplots()
