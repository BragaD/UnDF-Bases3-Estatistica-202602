...
- **INV-3: Escopo.** O livro tem a Introdução, o Cap. 1 e o Cap. 2 (Probabilidade) em construção; os antigos Caps. 2 a 5 foram removidos em 2.2026. Do Cap. 1 foram cortados: curtose, violino, curva de densidade, convenções `lower`/`higher` de quantil, valor esperado (1.6) e heatmap (1.7); a 1.8 é leitura complementar, fora do cronograma e do notebook. Probabilidade vem de @bussab2023 caps. 5–7, em três capítulos do livro (2, 3 e 4), com o escopo por seção fixado no `CLAUDE.md` ("Próximo capítulo"); 6.6.5 (Poisson) e 7.4.3 (exponencial) são suplementares, fora do cronograma e do notebook.
