...
| Nome | Valores de $p$ | Quantos cortes |
|---|---|---|
| Mediana | $0{,}5$ | 1 |
| Quartis | $0{,}25;\ 0{,}50;\ 0{,}75$ | 3 |
| Decis | $0{,}1;\ 0{,}2;\ \ldots;\ 0{,}9$ | 9 |
| Percentis | $0{,}01;\ 0{,}02;\ \ldots;\ 0{,}99$ | 99 |

::: {.callout-note}
A definição de quantil de ordem $p$ e os nomes particulares seguem a seção 3.3 de @bussab2023.
:::

O **percentil** $P$ é, portanto, o mesmo que $q(P/100)$: o percentil 95 é $q(0{,}95)$, e o percentil 50 é a mediana. Percentil e quantil não são conceitos diferentes — é a mesma ideia em duas escalas, porcentagem e proporção. O `quantile()` do pandas usa a segunda: pede-se `0.95`, não `95`.
...
## Qual é o quantil, afinal?

A definição acima — "o valor tal que $100p\%$ das observações são menores" — só é realizável ao pé da letra quando $np$ dá um número inteiro de observações. Quando não dá, **não existe** valor algum que satisfaça a definição, e é preciso escolher um.

A seção 1.4 já esbarrou nisso: com $n = 27$, a posição do primeiro quartil é 7,5, e o valor saiu da média entre a sétima e a oitava observações. Aquela foi *uma* escolha entre várias. O pandas oferece cinco, pelo argumento `interpolation`:

```{python}
#| label: convencoes-estados
convencoes = ["linear", "lower", "higher", "nearest", "midpoint"]
comp = pd.DataFrame(
    {c: estado["Populacao"].quantile([0.25, 0.75], interpolation=c) for c in convencoes}
).T
comp.columns = ["q1", "q3"]
comp["dq"] = comp["q3"] - comp["q1"]
comp.map(lambda v: num(v, 0))
```

Cinco respostas diferentes para "qual é a distância interquartil da população dos estados", da menor (`{python} num(comp["dq"].min(), 0)`) à maior (`{python} num(comp["dq"].max(), 0)`) — uma diferença de `{python} num(100 * (comp["dq"].max() / comp["dq"].min() - 1), 1)`%. Nenhuma está errada: `lower` toma sempre a observação de baixo, `higher` a de cima, `nearest` a mais próxima, `midpoint` o ponto médio entre as duas, e `linear` — o padrão — interpola proporcionalmente à distância. Como aqui as posições caem exatamente no meio do caminho, `linear` e `midpoint` coincidem.
...
Onde o boxplot mostra quartis, o violino mostra a forma inteira: é a ferramenta que revelaria, por exemplo, dois "ventres" separados por um estrangulamento no meio — uma bimodalidade que a caixa do boxplot esconderia por completo, já que ela só enxerga Q1, mediana e Q3.

::: {.callout-note}
**Ressalva do pythonbook (@weed):** a densidade do violino é uma **estimativa suavizada**, não os dados crus. Essa suavização pode sugerir massa onde não há observação nenhuma — a cauda do violino se estende além do maior e do menor valor reais, insinuando extremos que não existem. Com poucos dados, como os 27 estados desta seção, esse efeito engana mais do que informa; o violino compensa melhor quando há muitas observações para a densidade estimar com confiança.
:::
...
```{python}
#| label: fig-boxplot-assimetria
#| fig-cap: "As mesmas três turmas em boxplot. A linha central é a mediana; a caixa vai do primeiro ao terceiro quartil; os bigodes alcançam o restante das notas."
...
::: {.conceito}
O número que o `stats.skew(..., bias=False)` imprime não é exatamente o coeficiente de momentos $g_1$ da fórmula acima, e sim o estimador **corrigido de viés** $G_1$, que multiplica $g_1$ pelo fator $\frac{\sqrt{n(n-1)}}{n-2}$. Com $n = 27$ esse fator é um pouco maior que 1, então o **2,96** impresso fica ligeiramente acima do que a fórmula "crua" $g_1$ daria — a correção compensa a tendência de $g_1$ a subestimar a assimetria em amostras pequenas. Já a **curtose** logo abaixo segue outra convenção — o *excesso* padrão, sem essa correção de viés. São estimadores diferentes para coisas diferentes, e é normal que cada medida siga a sua convenção; só vale não estranhar que uma venha corrigida e a outra não.
:::
...
O `stats.kurtosis` do scipy calcula o **excesso** de curtose: o valor já sai descontado de 3, de modo que uma distribuição normal dá exatamente 0 — não 3, como na definição "crua" do quarto momento. Positivo significa caudas mais pesadas que a normal; negativo, caudas mais leves.

A população das UFs tem curtose **8,78** — um valor enorme, e a razão é a mesma de sempre: São Paulo. Um único estado tão distante do resto não só arrasta a média (seção 1.3) e desequilibra a assimetria (acima) — ele também infla a curtose, porque o quarto momento eleva os desvios à quarta potência, e um desvio grande, elevado a 4, domina a soma inteira. A taxa de homicídios tem curtose **-0,88**: negativa, ou seja, caudas mais leves que a normal, uma distribuição mais "achatada", sem o extremo isolado que a curtose positiva denuncia.

Fica a antecipação: o Capítulo 2 (seção 2.8) volta à curtose para caracterizar caudas longas de forma mais completa — lá ela deixa de ser só um número solto e vira parte do diagnóstico de quando a normal deixa de ser um modelo razoável para os dados.
...
**5.** Um boxplot e um gráfico de violino da mesma variável mostram a mesma mediana e o mesmo IQR — mas o violino tem dois "ventres" largos separados por um estrangulamento estreito no meio. O que o violino está mostrando que o boxplot esconde? E por que essa mesma leitura seria arriscada se a variável tivesse só 8 observações em vez de centenas?

::: {.callout-tip collapse="true"}
## Resposta

O violino está mostrando **bimodalidade**: dois agrupamentos de valores separados por uma faixa com poucas ou nenhuma observação. O boxplot não tem como revelar isso — ele só registra Q1, mediana e Q3, e esses três números podem ser idênticos para uma distribuição unimodal e para uma bimodal. Com poucas observações, porém, a curva do violino é uma estimativa suavizada, não os dados crus: com 8 pontos, o suavizador pode desenhar dois ventres (ou esticar as caudas além do mínimo e do máximo reais) só por acaso da amostra, sem que a bimodalidade exista de fato na população. O violino informa mais que o boxplot quando há dados suficientes para a densidade estimada ser confiável — e engana mais do que informa quando não há.
:::

## Agora é com você

De volta aos 10.692 aluguéis das seções [1.3](03-estimativas-localizacao.qmd) e [1.4](04-estimativas-variabilidade.qmd). Você já sabe onde está o centro e quanto os valores se espalham; falta a **forma** — e é nela que este conjunto mais surpreende.

```{python}
#| label: alugueis-dist-carrega
...
```{python}
#| label: alugueis-dist-percentis
colunas = ["aluguel", "condominio"]
percentis = [0.05, 0.25, 0.5, 0.75, 0.95]
tabela = alugueis[colunas].quantile(percentis)
tabela.index = [f"{p:.0%}" for p in percentis]
tabela.loc["máximo"] = alugueis[colunas].max()
tabela.astype(int)
```

Em `aluguel`, 95% dos imóveis custam até R$ `{python} num(tabela.loc["95%", "aluguel"], 0)`, e o máximo é R$ `{python} num(tabela.loc["máximo", "aluguel"], 0)` — cerca de `{python} num(tabela.loc["máximo", "aluguel"] / tabela.loc["95%", "aluguel"], 1)` vezes o P95. Em `condominio` a distância é de outra ordem: P95 de R$ `{python} num(tabela.loc["95%", "condominio"], 0)` contra um máximo de R$ `{python} num(tabela.loc["máximo", "condominio"], 0)`, `{python} num(tabela.loc["máximo", "condominio"] / tabela.loc["95%", "condominio"], 0)` vezes maior. Os 5% finais de uma distribuição de cauda longa não são "um pouco maiores" — são outro mundo, e o percentil 95 é justamente a ferramenta para resumir "o típico entre os altos" sem deixar o máximo falar sozinho.
:::
...
```{python}
#| label: alugueis-dist-boxplot
#| fig-cap: "Boxplot do aluguel. A caixa é o IQR; os pontos acima do bigode são os outliers pela regra de 1,5×IQR."
q1, q3 = alugueis["aluguel"].quantile([0.25, 0.75])
limite_sup = q3 + 1.5 * (q3 - q1)
limite_inf = q1 - 1.5 * (q3 - q1)
acima = alugueis["aluguel"] > limite_sup
abaixo = alugueis["aluguel"] < limite_inf
print(f"Q1 = {num(q1, 0)}   Q3 = {num(q3, 0)}   IQR = {num(q3 - q1, 0)}")
print(f"limite superior = {num(limite_sup, 0)} → {int(acima.sum())} outliers ({num(100 * acima.mean(), 1)}%)")
print(f"limite inferior = {num(limite_inf, 0)} → {int(abaixo.sum())} outliers")

fig, ax = plt.subplots(figsize=(4, 5))
alugueis["aluguel"].plot.box(ax=ax)
ax.set_ylabel("Aluguel (R$)")
plt.tight_layout()
plt.show()
```

`{python} int(acima.sum())` imóveis — `{python} num(100 * acima.mean(), 1)`% do conjunto — passam do bigode superior (R$ `{python} num(limite_sup, 0)`). Pelo lado de baixo, `{python} int(abaixo.sum())`: o limite inferior dá R$ `{python} num(limite_inf, 0)`, e não existe aluguel tão barato. Essa assimetria do próprio boxplot — bigode curto embaixo, chuva de pontos em cima — já é a assinatura visual da cauda longa à direita. E note a proporção: num dado normal, a regra de 1,5×IQR marca menos de 1% dos valores como outlier; aqui marca `{python} num(100 * acima.mean(), 1)`%. Quando "outlier" vira uma fatia visível do conjunto, o problema não está nos pontos — está em esperar simetria de um dado que não a tem.
:::
...
```{python}
#| label: alugueis-dist-faixas-bruto
faixas_bruto = pd.cut(alugueis["area_m2"], 10).value_counts().sort_index()
faixas_bruto
```

Dez faixas de largura igual entre o mínimo e `{python} num(alugueis["area_m2"].max(), 0)` m² dão faixas de cerca de `{python} num(alugueis["area_m2"].max() / 10, 0)` m² cada — e `{python} num(faixas_bruto.iloc[0], 0)` dos `{python} num(len(alugueis), 0)` imóveis caem na primeira. Não é um histograma; é uma barra e nove faixas quase vazias. Um único valor absurdo dita a escala inteira, e a forma real dos outros dez mil desaparece.

```{python}
#| label: alugueis-dist-faixas-filtrado
corte = alugueis["area_m2"].quantile(0.99)
ate_corte = alugueis.loc[alugueis["area_m2"] <= corte, "area_m2"]
print(f"percentil 99 = {num(corte, 0)} m² → {num(len(ate_corte), 0)} imóveis dentro do corte")
pd.cut(ate_corte, 10).value_counts().sort_index()
```

```{python}
#| label: alugueis-dist-hist
#| fig-cap: "Histograma da área só até o percentil 99: a forma aparece — concentração nas áreas pequenas e cauda longa à direita, agora legível."
fig, ax = plt.subplots()
ate_corte.plot.hist(ax=ax, bins=20, edgecolor="white")
...
Deixando de fora 1% dos imóveis (os `{python} num(len(alugueis) - len(ate_corte), 0)` acima de `{python} num(corte, 0)` m²), as faixas encolhem para cerca de `{python} num(corte / 10, 0)` m² e o histograma passa a mostrar uma distribuição: muitos imóveis pequenos, cada vez menos à medida que a área cresce. Cuidado com o que isso é e o que não é: um **corte para visualizar**, declarado e reversível — não uma limpeza dos dados. O 1% continua existindo; só não pode escolher a escala do gráfico sozinho.
:::
...
```{python}
#| label: alugueis-dist-assimetria
colunas = ["aluguel", "condominio", "area_m2"]
forma = pd.DataFrame({
    "média": alugueis[colunas].mean().round(0).astype(int),
    "mediana": alugueis[colunas].median().round(0).astype(int),
    "assimetria": pd.Series([round(stats.skew(alugueis[c], bias=False), 1) for c in colunas], index=colunas),
})
forma
```

As três assimetrias são positivas, e nas três a média fica acima da mediana — a regra da seção, confirmada em dado real. O que muda é o patamar: `aluguel` tem assimetria `{python} num(forma.loc["aluguel", "assimetria"], 1)` (cauda à direita, mas de um dado que ainda "parece" aluguel), enquanto `area_m2` chega a `{python} num(forma.loc["area_m2", "assimetria"], 1)`. O motivo é o mesmo imóvel de `{python} num(alugueis["area_m2"].max(), 0)` m² das tarefas anteriores: a assimetria eleva os desvios ao **cubo**, então um único valor a centenas de desvios-padrão da média domina a estatística inteira. Assimetria alta pode significar "distribuição genuinamente inclinada" ou "um erro de digitação" — e a estatística sozinha não distingue os dois. O boxplot da tarefa 2 e o histograma da tarefa 3 distinguem.
:::

Fechando com o mesmo dado das três últimas seções: a **forma** de uma distribuição é o que explica por que média e mediana discordam (1.3) e por que desvio-padrão e MAD discordam (1.4). Percentis, boxplot e histograma não são enfeite — são como se descobre, antes de qualquer conta, que o dado tem cauda e onde ela começa.
