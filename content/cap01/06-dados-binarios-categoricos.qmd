...
::: {.conceito}
Repare no formato de `dfw`: uma linha, cinco colunas. Cada **coluna** é uma categoria, e o único valor de cada uma é a contagem de atrasos atribuídos a ela. É o oposto do formato "uma linha por observação" que os capítulos anteriores usaram. É por isso que os próximos chunks recorrem a `.transpose()`: para virar as cinco colunas em cinco linhas, uma por causa, antes de desenhar o gráfico ou extrair a moda.
:::

`ControleAereo` e `VooAnterior` somam sozinhos mais de 70% dos atrasos; `Seguranca` responde por pouco mais de um décimo de 1%. É toda a "distribuição" que um dado categórico tem para oferecer: um conjunto de fatias, sem forma contínua.
...
#| fig-alt: "Gráfico de barras separadas com as cinco causas de atraso no eixo horizontal e o número de atrasos no vertical. VooAnterior tem a barra mais alta, seguida de ControleAereo e Companhia; Clima é baixa e Seguranca é praticamente invisível."
fig, ax = plt.subplots()
...
::: {.exemplo}
A moda de `dfw` é `VooAnterior`, com 42% dos atrasos, à frente de `Companhia` e `Clima`, as causas que viriam à mente primeiro ao pensar em "atraso de voo". A maior fonte de atraso em Dallas/Fort Worth não é uma causa local do próprio aeroporto: é um voo anterior da mesma aeronave chegando atrasado e empurrando o atraso adiante. O gargalo fica fora de Dallas, na malha aérea inteira, e se propaga de aeroporto em aeroporto.
:::
...
**1.** Por que a moda é a única medida de localização que faz sentido para `dfw`?
