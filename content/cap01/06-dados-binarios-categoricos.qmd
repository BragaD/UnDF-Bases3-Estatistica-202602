...
Mas essa razão é a das **alturas desenhadas**, não das taxas: `(36,78 − 20) / (20,47 − 20)`, que dá ≈36. A razão **real** entre as taxas é `36,78 / 20,47` = **1,8×**. Pernambuco tem uma taxa 80% maior que a do Piauí — não 36 vezes maior. É uma diferença real, que merece ser levada a sério; só não é a diferença que o painel da esquerda mostra.

Por que a barra engana especificamente: num gráfico de barras, o olho lê a quantidade pelo **comprimento** da barra, e comprimento só significa algo medido **a partir do zero**. Cortar o eixo em 20 quebra essa codificação — a barra do Piauí ainda existe, mas o pedaço que sobrou para representá-la é quase nada, mesmo a taxa real sendo bem acima de zero.

O que torna esse truque particularmente insidioso é que **todos os nove números são verdadeiros.** Não há dado inventado, não há erro de conta — só um eixo que começa em 20 em vez de em 0. É assim que se engana com uma base de dados honesta: não mentindo sobre os valores, mas escolhendo onde a régua começa a medir.

O painel da direita mostra a imagem real: todas as nove barras são altas — todos os estados do Nordeste têm taxa acima de 20 por 100 mil, o que por si só já é uma informação (a região inteira convive com violência letal alta) — e as diferenças entre elas aparecem na proporção verdadeira, bem mais modesta do que o primeiro painel sugeria.
