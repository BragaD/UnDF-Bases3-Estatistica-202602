...
A **variância** é a média dos desvios ao quadrado, e o **desvio-padrão** é a sua raiz quadrada:
...
|---|---|---|
| Desvio-padrão | `{python} f"{desvio:,.0f}"` | Não |
| IQR | `{python} f"{iqr:,.0f}"` | Sim |
| MAD | `{python} f"{mad:,.0f}"` | Sim |

::: {.exemplo}
Se a população de cada estado fosse multiplicada por 1.000 — trocando a unidade de pessoas para "milhares de pessoas" —, o desvio-padrão e o IQR também seriam multiplicados por 1.000: ambos herdam a unidade dos dados e escalam linearmente com eles. A variância, por estar em unidade ao quadrado, seria multiplicada por 1.000.000. É exatamente esse descasamento de unidade que faz o desvio-padrão, e não a variância, ser reportado na prática.
:::
